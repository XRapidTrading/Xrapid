...
async def create_sniper_config(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Format settings display without parse_mode to avoid escaping issues
    settings_display = "\n".join([f"  • {key}: {value}" for key, value in current_settings.items()]) if current_settings else "  No settings configured yet"
    message_text = f"Configure your sniping settings:\n\nCurrent settings:\n{settings_display}"
...
async def generate_wallet(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if "error" not in new_wallet_data:
        ...
        await update.callback_query.edit_message_text(
            ...
            f"\n\nYour wallet is now set up. Use the menu to view details."
        )
    else:
